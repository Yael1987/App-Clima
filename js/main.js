document.addEventListener("DOMContentLoaded", () => {
  ...
  const currentWeatherTemp = document.querySelector(".description__temp");
  const currentWeatherTextPrimary = document.querySelector(
    ".description__text-primary"
  );
  const currentWeatherTextSecondary = document.querySelector(
    ".description__text-secondary"
  );
...
function displayHourlyWeather(hourly) {
  ...
  hourly.forEach((weatherEl) => {
    ...
    htmlRes += `
      <div class="weather-hour">${hour < 10 ? "0" + hour : hour}:00 / ${
      weatherEl.temp
    }&deg;</div>
    `;
  });
